...
{    
    ...
    OCL_SAFE_CALL(clGetDeviceInfo(gpu_id, CL_DEVICE_NAME, 0, nullptr, &deviceNameSize));

    std::vector<unsigned char> deviceName(deviceNameSize, 0);
    OCL_SAFE_CALL(clGetDeviceInfo(gpu_id, CL_DEVICE_NAME, deviceNameSize, deviceName.data(),nullptr));
    ...
    cl_context context = clCreateContext(nullptr, 1, &gpu_id, nullptr, nullptr, &err_code);
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &str, nullptr, &err_code);
    OCL_SAFE_CALL(err_code);

    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    OCL_SAFE_CALL(clBuildProgram(program, 1, &gpu_id, nullptr, nullptr, nullptr));
    ...
    size_t log_size = 0;
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, gpu_id, CL_PROGRAM_BUILD_LOG, 0,
                                             nullptr, &log_size));

    std::vector<char> log(log_size, 0);
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, gpu_id, CL_PROGRAM_BUILD_LOG, log_size,
                                             log.data(), nullptr));
    if (log_size > 1) {
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_work_size, &workGroupSize,
                                   0, nullptr, &event);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, cs_gpu, CL_TRUE, 0, sizeof(float) * n,
